...
│  Service (비즈니스 로직)                                                 │
│      │                                                                  │
│      ├── Repository (JPA) ◄────── Command (저장, 수정, 삭제)            │
│      │                                                                  │
│      └── Mapper (MyBatis) ◄─────── Query (조회)                         │
│                                                                         │
│  Entity.createForXxx() ◄───────── Factory Method (엔티티 생성)          │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
            ┌───────────────────────┴───────────────────────┐
            │                                               │
            ▼                                               ▼
┌───────────────────────────────┐       ┌───────────────────────────────┐
│  Infrastructure Layer (JPA)   │       │ Infrastructure Layer (MyBatis)│
├───────────────────────────────┤       ├───────────────────────────────┤
│  Repository                   │       │  Mapper                       │
│  - save(), existsBy*()        │       │  - findByXxx()                │
│  - countBy*()                 │       │  - selectList()               │
└───────────────────────────────┘       └───────────────────────────────┘
                │                                       │
                └───────────────────┬───────────────────┘
...
- Port/Adapter 패턴은 파일 증가(4개+)로 복잡도 상승
- CQRS로 이미 Command/Query가 기술적으로 분리됨
- Factory Method로 엔티티 생성 로직을 충분히 캡슐화
...
| 예약 취소                 | `ums_msg` 직접 DELETE           | MSG_STATUS='ready' 상태만 취소 가능       |
| 재발송                    | SNAP Agent 처리                 | 구현 불필요                               |
| 통계                      | 별도 테이블 (`ums_stats_daily`) | 일 10만건+ 대응, 빠른 조회                |

> **상세 문서**: [SNAP Agent 분석 보고서](./SNAP-AGENT-ANALYSIS.md), [UMS 메시지 발송 가이드](./implementation-guides/06-UMS-MESSAGE-FLOW.md)

### 2.2 데이터베이스 구조

**단일 MySQL 데이터베이스 전략 (초기 구현)**

프로토타입 및 초기 개발 단계에서는 단일 MySQL 데이터베이스를 사용하되, 테이블을 논리적으로 분리하여 향후 확장성을 유지합니다.

**사용자 관리 영역**:

- 사용자 인터페이스 및 부가 기능 지원
- 테이블: 주소록, 발신번호, 보관함, 공유 주소록, 고객 리스트, 감사 로그

**메시지 처리 영역**:

- 메시지 발송 처리 및 이력 관리
- 테이블: ums*msg, ums_log*{YYYYMM}
- 대용량 쓰기에 최적화된 인덱스 구조

**향후 확장 계획**:

- 트래픽이 증가하면 물리적으로 데이터베이스 분리 가능
- 현재 논리적 분리로 마이그레이션 용이성 확보
- 별도 데이터 동기화 모듈 불필요 (단순화)
...
│       │   ├── SignUpRequest.java
│       │   ├── LoginRequest.java
│       │   ├── UserLoginQueryDto.java      # MyBatis Query DTO
...
│   ├── user/
│   │   ├── User.java                       # Entity + createForSignUp()
│   │   ├── UserType.java
│   │   ├── UserRole.java
│   │   └── UserStatus.java
│   ├── organization/
...
│   ├── jpa/
│   │   └── repository/                     # Command (JPA)
│   │       ├── UserRepository.java
│   │       └── OrganizationRepository.java
│   └── mybatis/
│       └── mapper/                         # Query (MyBatis)
...
| **Infrastructure** | `infra/jpa/repository/` | Command (JPA)                      |
| **Infrastructure** | `infra/mybatis/mapper/` | Query (MyBatis)                    |
...
CREATE TABLE users (
    ...
    user_type VARCHAR(20) NOT NULL COMMENT 'INDIVIDUAL|BUSINESS',
...
CREATE TABLE ums_msg (
    ...
    -- 예비 필드 (LinkWave 활용)
    ETC1 VARCHAR(50) COMMENT '사용자 ID',
    ETC2 VARCHAR(50) COMMENT '조직 ID',
    ETC3 VARCHAR(50) COMMENT '발송 그룹 ID',
    ETC4 VARCHAR(50) COMMENT '발송 타입 (IMMEDIATE/SCHEDULED/BULK)',
    ETC5 VARCHAR(50) COMMENT '캠페인 ID',
    ETC6 VARCHAR(50) COMMENT '예비',

    -- 인덱스
    INDEX idx_msg_status (MSG_STATUS),
    INDEX idx_etc3_status (ETC3, MSG_STATUS) COMMENT '그룹별 취소 조회용',
...
CREATE TABLE ums_log_202512 (
    ...
    INDEX idx_etc1_done_date (ETC1, DONE_DATE) COMMENT '사용자별 통계 집계용'
) COMMENT='메시지 발송 이력';
```

#### 5.2.3 ums_stats_daily (일별 통계)
...
- **발송 요청 시**: `total_count`, `pending_count` 증가
- **예약 취소 시**: `pending_count` 감소
- **결과 수신 시**: `@Scheduled` 배치로 `success_count`, `fail_count` 집계
...
public class MessageService {
    ...
    @Transactional
    public MessageResponse sendMessage(MessageRequestDto request, String userId) {
        // 1. 발신번호 검증
        validateSenderNumber(request.getSenderNumber(), userId);
        ...
        // 5. DEDUP_HASH 생성
        String dedupHash = dedupService.generateHash(
            request.getRecipients().get(0).getPhone(),
            request.getContent()
        );

        // 6. UmsMsg 엔티티 생성 및 저장
        for (RecipientDto recipient : request.getRecipients()) {
            UmsMsg umsMsg = UmsMsg.builder()
                ...
                .dedupHash(dedupHash)
                .mmsFileList(request.getFiles() != null ?
                             String.join(",", uploadFiles(request.getFiles())) : null)
                .build();
...
spring:
  ...
  datasource:
    url: ${DB_URL:jdbc:mysql://localhost:3306/linkwave?useSSL=false&serverTimezone=Asia/Seoul}
...
## 부록 A: JPA & MyBatis 하이브리드 패턴

### A.1 도메인별 기술 분리 전략

| 도메인             | 기술    | 테이블                                                     | 특징                      |
| ------------------ | ------- | ---------------------------------------------------------- | ------------------------- |
| **User Domain**    | JPA     | `users`, `organizations`, `sender_numbers`, `address_book` | 관계 중심, CRUD 중심      |
| **Message Domain** | MyBatis | `ums_msg`, `ums_log_{YYYYMM}`, `ums_stats_daily`           | 대용량, 복잡한 쿼리, 통계 |

**핵심 원칙**:

- ✅ 같은 도메인 내에서는 하나의 기술만 사용
- ✅ User 테이블은 JPA로만 접근
- ✅ ums_msg 테이블은 MyBatis로만 접근
- ⚠️ 크로스 도메인 쿼리 시 주의 필요
...
```java
@Transactional(readOnly = true)  // ✅ readOnly = true 필수
public UserStatisticsDto getUserStatistics(UUID userId) {
    // 1. JPA로 사용자 정보 조회
    User user = userRepository.findById(userId).orElseThrow();

    // 2. MyBatis로 메시지 통계 조회
    MessageStatistics stats = umsMsgMapper.getStatisticsByUserId(userId);

    // 3. DTO 조합
    return UserStatisticsDto.from(user, stats);
}
```

#### 패턴 2: JPA 수정 후 MyBatis 조회 (⚠️ flush 필수)

```java
@Transactional
public MessageDetailDto viewMessage(String clientKey, UUID userId) {
    // 1. JPA로 사용자 활동 시간 업데이트
    User user = userRepository.findById(userId).orElseThrow();
    user.updateLastViewedAt(LocalDateTime.now());

    // 2. ⚠️ 필수: MyBatis 조회 전에 flush
    entityManager.flush();

    // 3. MyBatis로 메시지 조회
    return umsMsgMapper.getMessageDetail(clientKey);
}
```
...
```java
@Transactional
public void updateStatisticsBatch(int batchSize) {
    int page = 0;
    Page<User> userPage;

    do {
        userPage = userRepository.findAll(PageRequest.of(page++, batchSize));
        List<UUID> userIds = userPage.stream()
            .map(User::getUserId).toList();

        // 배치로 통계 조회
        Map<UUID, MessageStatistics> statsMap = umsMsgMapper
            .getBulkStatisticsByUserIds(userIds).stream()
            .collect(Collectors.toMap(MessageStatistics::getUserId, Function.identity()));

        // 업데이트
        for (User user : userPage) {
            user.updateStats(statsMap.get(user.getUserId()));
        }
...
public class TransactionConfig {
    ...
    @Primary  // ⚠️ 필수: JPA + MyBatis 롤백 보장
...
```java
// ❌ 절대 금지: 같은 테이블을 두 기술로 수정
@Transactional
public void badPattern(UUID userId) {
    User user = userRepository.findById(userId).orElseThrow();
    user.setName("홍길동");  // JPA

    userMapper.updateUserPhone(userId, "010-1234-5678");  // MyBatis
    // ❌ 문제: User 테이블은 JPA로만 접근해야 함
}
```
...
### B.1 Spring Boot 4.0 선택 이유

- 최신 Spring 생태계 지원
- Java 17+ 지원
- 향상된 성능 및 보안

### B.2 JPA/Hibernate 선택 이유
